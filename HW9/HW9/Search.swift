                     ...
                     case .success(let value):
                        ...
                        var ind =  Int(String(diff[diff.startIndex]))!
                         ...
                         let article2 = article(image:image,title:title1,time:thrs,section:sectionname,articleid:articleid,url:url)
                             ...
                             self.article10.append(article2)
                            
                         
                             sid.append(json[i]["id"].stringValue)
                             i=i+1
                            print(diddate)
                         }
...
extension Search: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        let configuration = UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { actions -> UIMenu? in
            let twitter = UIAction(title: "Share with Twitter", image: UIImage(named: "twitter"), identifier: nil) { action in
                let textString = self.article10[indexPath.row].url + "#CSCI_571_NewsApp"
                let tweetUrl = self.article10[indexPath.row].title
            ...
            let userdefaults = UserDefaults.standard
                                  ...
                                  for datas in JSON(data)
                                  {
                                      ...
                                      if(self.article10[indexPath.row].articleid == datas.1["id"].string!)
                                      {
                                          flag = true
                                          break;
                                          }
                    ...
                    let userdefaults = UserDefaults.standard
                                var data = userdefaults.array(forKey: "bookmark")
                                    ...
                                    if(data?.count == 0 || data == nil )
                                    {
                                        ...
                                        var data2  = [
                                            "id" : self.article10[indexPath.row].articleid,
                                           "image" :self.article10[indexPath.row].image,
                                            "section" :self.article10[indexPath.row].section,
                                            "date": diddate[indexPath.row],
                                            "title": self.article10[indexPath.row].title,
                                            "url" : self.article10[indexPath.row].url
                                        ]
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
      ...
      override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
          if segue.identifier == "showsa" {
              
              
              if(segue.destination.isKind(of: DSA.self))

              {
                
                  let secondVC = segue.destination as! DSA

                  //let indexPath = sender as! IndexPath
                  //debugPrint(indexselected)
               // print(article10[indexselected].articleid)
                  secondVC.data = aid[indexselected]
                  secondVC.titlenav = article10[indexselected].title
                secondVC.id = article10[indexselected].articleid
              }
          }
      }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        cell.HomeBookmark.tag = indexPath.row
        var bdata = UserDefaults.standard.array(forKey: "bookmark")
        //print(bdata)
        var f = false
        for x in JSON(bdata){
            
            if(x.1["id"].stringValue == self.article10[indexPath.row].articleid)
            {
                f = true
                break;
            }
        ...
        if( f == true)
        {
            cell.HomeBookmark.setImage(UIImage(systemName: "bookmark.fill"), for: UIControl.State.normal)
        }
        else{
             cell.HomeBookmark.setImage(UIImage(systemName: "bookmark"), for: UIControl.State.normal)
        }
        cell.HomeBookmark.addTarget(self, action: #selector(add), for: UIControl.Event.touchUpInside)
        //cell.ArticleImage.image=UIImage(named: "clear_weather")
//        cell.setitem(article1: video)
        cell.SearchTitle.text = article10[indexPath.row].title
        cell.SearchTime.text = article10[indexPath.row].time
        cell.SearchSection.text = article10[indexPath.row].section
            
        
           let url = NSURL(string:article10[indexPath.row].image)
           ...
           if imagedata != nil {
               cell.SearchImage.image = UIImage(data:imagedata! as Data)
                ...
                cell.SearchImage.image = UIImage(named:"default-guardian" )
            }
    ...
    @objc func add(sender:UIButton){
           ...
           var data = userdefaults.array(forKey: "bookmark")
               ...
               if(data?.count == 0 || data == nil )
               {
                    self.view.makeToast("Article Bookmarked. Check out the Bookmarks tab to view", duration: 2.0)
                   var data2  = [
                       "id" : self.article10[rownumber].articleid,
                       "image" :self.article10[rownumber].image,
                       "section" : self.article10[rownumber].section,
                       "date": diddate[rownumber],
                       "title": self.article10[rownumber].title,
                       "url" : self.article10[rownumber].url
                   ]
                   userdefaults.set([data2], forKey: "bookmark")
